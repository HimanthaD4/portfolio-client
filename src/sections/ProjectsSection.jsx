...
const ProjectsSection = () => {
  ...
  const [placeholderCache] = useState({});

  // Generate SVG placeholder image
  const generatePlaceholder = (text = 'Project Preview') => {
    if (placeholderCache[text]) return placeholderCache[text];
    
    const svg = `
      <svg xmlns="http://www.w3.org/2000/svg" width="800" height="600" viewBox="0 0 800 600">
        <rect width="800" height="600" fill="#0f172a"/>
        <text x="400" y="300" fill="#e2e8f0" font-family="Arial" font-size="24" 
  ...
  const handleImageError = (e, title) => {
    e.target.src = generatePlaceholder(title);
    e.target.onerror = null; // Prevent infinite loop
  };

  const getImageSource = (project) => {
    if (!project.image) return generatePlaceholder(project.title);
    
    // Use URL if available (preferred method)
...
const ProjectCard = ({ project, isVisible, getImageSource, handleImageError, getCategoryIcon }) => {
  ...
  return (
    <motion.div
      ...
      whileHover={{ 
        y: -5, 
        borderColor: `${
          category === 'web' ? 'rgba(56, 189, 248, 0.3)' : 
          category === 'ai' ? 'rgba(236, 72, 153, 0.3)' : 
          'rgba(74, 222, 128, 0.3)'
        }`,
        boxShadow: `0 15px 30px ${
          category === 'web' ? 'rgba(56, 189, 248, 0.1)' : 
          category === 'ai' ? 'rgba(236, 72, 153, 0.1)' : 
          'rgba(74, 222, 128, 0.1)'
        }`
      }}
    >
      ...
      <div className="project-content">
        ...
        <div className="project-links">
          {project.github && (
            ...
            >
              <FaCode className="link-icon" /> Code
            </motion.a>
          )}
          <motion.a 
            href={project.live || '#'} 
          ...
          >
            <FaGlobe className="link-icon" /> {project.live ? 'Live' : 'N/A'}
          </motion.a>
        </div>
      </div>
    </motion.div>
